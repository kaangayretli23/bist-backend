"""
BIST Pro v3.0.2 - RENDER BULLETPROOF
KURAL: Hicbir API endpoint'i yfinance CAGIRMAZ.
Tum veri background thread'den gelir, API sadece cache okur.
"""
...
_stock_cache = {}   # sym -> {data, ts}
_index_cache = {}   # key -> {data, ts}
_hist_cache = {}    # sym_period -> {data, ts}
_status = {'phase': 'idle', 'loaded': 0, 'total': 0, 'lastRun': None, 'errors': []}
CACHE_TTL = 600  # 10 dk
...
# ---- BACKGROUND LOADER ----
def _load_one_stock(sym):
    try:
        h = yf.Ticker(f"{sym}.IS").history(period="5d", timeout=8)
        if h is None or h.empty or len(h) < 2:
            return None
        cur = sf(h['Close'].iloc[-1])
        prev = sf(h['Close'].iloc[-2])
        if prev == 0: return None
        ch = sf(cur - prev)
        return {
            'code': sym, 'name': BIST100_STOCKS.get(sym, sym),
            'price': cur, 'prevClose': prev, 'change': ch,
            'changePct': sf(ch / prev * 100),
            'volume': si(h['Volume'].iloc[-1]),
            'open': sf(h['Open'].iloc[-1]),
            'high': sf(h['High'].iloc[-1]),
            'low': sf(h['Low'].iloc[-1]),
            'gap': sf(sf(h['Open'].iloc[-1]) - prev),
            'gapPct': sf((sf(h['Open'].iloc[-1]) - prev) / prev * 100),
        }
    except Exception as e:
        print(f"  STOCK {sym}: {e}")
        return None

def _load_one_index(key, tsym, name):
    try:
        h = yf.Ticker(tsym).history(period="5d", timeout=8)
        if h is None or h.empty or len(h) < 2:
            return None
        cur = sf(h['Close'].iloc[-1], 4)
        prev = sf(h['Close'].iloc[-2], 4)
        return {
            'name': name, 'value': cur,
...
def _load_stock_hist(sym, period='1y'):
    """Tek hisse icin detayli gecmis - cache'li"""
    ck = f"{sym}_{period}"
    cached = _cget(_hist_cache, ck)
    if cached is not None:
        return cached
    try:
        h = yf.Ticker(f"{sym}.IS").history(period=period, timeout=12)
        if h is not None and not h.empty and len(h) >= 2:
            _cset(_hist_cache, ck, h)
            return h
    except Exception as e:
        print(f"  HIST {sym}: {e}")
    return None
...
def _background_loader():
    """Arka plan: once endeksler, sonra BIST30 hisseleri"""
    if not YF_OK:
        print("[LOADER] yfinance yok")
        return
    ...
    while True:
        try:
            ...
            print("[LOADER] Endeksler yukleniyor...")
            for key, (tsym, name) in INDEX_TICKERS.items():
                data = _load_one_index(key, tsym, name)
                if data:
                    _cset(_index_cache, key, data)
                    print(f"  [IDX] {key} OK")
                time.sleep(0.5)
            ...
            _status['loaded'] = 0
            _status['errors'] = []
            print(f"[LOADER] Hisseler yukleniyor ({len(BIST30)})...")
            for i, sym in enumerate(BIST30):
                data = _load_one_stock(sym)
                if data:
                    _cset(_stock_cache, sym, data)
                else:
                    _status['errors'].append(sym)
                _status['loaded'] = i + 1
                if (i + 1) % 5 == 0:
                    print(f"  [STOCK] {i+1}/{len(BIST30)} tamamlandi")
                time.sleep(0.5)

            _status['phase'] = 'done'
            _status['lastRun'] = datetime.now().isoformat()
            loaded = len(BIST30) - len(_status['errors'])
            print(f"[LOADER] Tamamlandi: {loaded}/{len(BIST30)} hisse, {len(_index_cache)} endeks")

        except Exception as e:
            print(f"[LOADER] FATAL: {e}")
            _status['phase'] = 'error'

        # 5 dk bekle, tekrar yukle
        print("[LOADER] 5 dk sonra tekrar yuklenecek...")
        time.sleep(300)
...
def _loading_response(endpoint=""):
    """Veri henuz hazir degilse standart yukleniyor yaniti"""
    return jsonify(safe_dict({
        'success': True, 'loading': True,
        'message': f"Veriler yukleniyor ({_status['loaded']}/{_status['total']})... Sayfayi 30sn sonra yenileyin.",
        'phase': _status['phase'],
    }))
...
def calc_rsi(closes, period=14):
    if len(closes) < period + 1:
        return {'name':'RSI','value':50.0,'signal':'neutral'}
    d = np.diff(closes)
    ...
    return {'name':'RSI','value':rsi,'signal':sig}
...
def calc_macd(closes):
    if len(closes) < 26:
        return {'name':'MACD','macd':0,'signal':0,'histogram':0,'signalType':'neutral'}
    ...
    return {'name':'MACD','macd':mv,'signal':sv,'histogram':sf(mv-sv),'signalType':sig}
...
def calc_bollinger(closes, cp, period=20):
    if len(closes) < period:
        return {'name':'Bollinger','upper':0,'middle':0,'lower':0,'signal':'neutral','bandwidth':0}
    r = closes[-period:]
    ...
    return {'name':'Bollinger','upper':u,'middle':m,'lower':lo,'bandwidth':sf((u-lo)/m*100 if m else 0),'signal':sig}
...
def calc_stochastic(closes, highs, lows, period=14):
    if len(closes)<period:
        return {'name':'Stochastic','k':50,'d':50,'signal':'neutral'}
    ...
    return {'name':'Stochastic','k':k,'d':k,'signal':sig}

def calc_atr(highs, lows, closes, period=14):
    if len(closes)<period+1:
        return {'name':'ATR','value':0,'pct':0,'signal':'neutral'}
...
def calc_adx(highs, lows, closes, period=14):
    n = len(closes)
    if n<period+1:
        return {'name':'ADX','value':25,'plusDI':0,'minusDI':0,'signal':'neutral'}
    tr, pdm, mdm = [], [], []
    for i in range(1,n):
    ...
    return {'name':'ADX','value':adx,'plusDI':pdi,'minusDI':mdi,'signal':sig}

def calc_obv(closes, volumes):
    if len(closes)<10:
        return {'name':'OBV','value':0,'trend':'neutral','signal':'neutral'}
    obv,vals=0,[0]
...
def calc_all_indicators(hist, cp):
    ...
    ov='buy' if bc>sc and bc>=t*0.4 else ('sell' if sc>bc and sc>=t*0.4 else 'neutral')
...
# =====================================================================
# ROUTES - HICBIRI YF CAGIRMAZ, TAMAMI CACHE'DEN OKUR
# =====================================================================

@app.route('/api/health')
def health():
    return jsonify({'status':'ok','version':'3.0.2','yf':YF_OK,'time':datetime.now().isoformat(),'loader':_status,'stockCache':len(_stock_cache),'indexCache':len(_index_cache)})
...
def dashboard():
    try:
        stocks = _get_stocks()
        if not stocks:
            return _loading_response("dashboard")
        ...
        return jsonify(safe_dict({
            'success':True,'loading':False,'stockCount':len(stocks),'timestamp':datetime.now().isoformat(),
...
def indices():
    """SADECE CACHE - yf cagirmaz"""
    try:
        data = _get_indices()
        if not data:
            return jsonify(safe_dict({'success':True,'loading':True,'indices':{},'message':'Endeks verileri yukleniyor...'}))
...
def bist100():
    try:
        ...
        if sector and sector in SECTOR_MAP:
            stocks = _get_stocks(SECTOR_MAP[sector])
        else:
            stocks = _get_stocks()
        if not stocks:
            return jsonify(safe_dict({'success':True,'stocks':[],'count':0,'sectors':list(SECTOR_MAP.keys()),'loading':True,'message':'Veriler yukleniyor...'}))
        rev=(order=='desc')
...
def stock_detail(symbol):
    """Tek hisse detay - BACKGROUND THREAD'de yfinance cagirir, request beklemez"""
    try:
        symbol = symbol.upper()
        ...
        if hist is None:
            # Cache'de yok - background'da yukle, simdilik bos don
            def _bg_load():
                _load_stock_hist(symbol, period)
            threading.Thread(target=_bg_load, daemon=True).start()

            # Cache'deki quick data'yi don
            quick = _cget(_stock_cache, symbol)
            if quick:
                return jsonify(safe_dict({
                    'success':True,'code':symbol,'name':quick['name'],
                    'price':quick['price'],'change':quick['change'],'changePercent':quick['changePct'],
                    'volume':quick['volume'],'dayOpen':quick['open'],'dayHigh':quick['high'],'dayLow':quick['low'],
                    'prevClose':quick['prevClose'],'currency':'TRY','period':period,
                    'loading':True,'message':'Detayli veri yukleniyor, sayfayi yenileyin...',
            ...
            return jsonify({'error':f'{symbol} verisi henuz yuklenmedi, sayfayi yenileyin','loading':True}), 202

        # Cache'de var - tam analiz yap
        ...
        info_data = {}
        try:
            info_data = _cget(_hist_cache, f"info_{symbol}") or {}
        except:
            pass

        return jsonify(safe_dict({
            'success':True,'code':symbol,
            'name':info_data.get('longName',BIST100_STOCKS.get(symbol,symbol)),
...
def stock_events(symbol):
    return jsonify(safe_dict({'success':True,'symbol':symbol.upper(),'events':{'dividends':[],'splits':[],'message':'Etkinlik verileri background yuklemede'}}))
...
def compare():
    try:
        ...
        for sym in symbols[:5]:
            s = _cget(_stock_cache, sym.upper())
            if s:
                results.append({'code':s['code'],'name':s['name'],'price':s['price'],'changePct':s['changePct'],'volume':s['volume']})
        return jsonify(safe_dict({'success':True,'comparison':results,'period':'5d'}))
    except Exception as e:
        return jsonify({'error':str(e)}), 500
...
def get_portfolio():
    ...
    for p in pf:
        ...
        mv,cb=cp*q,ac*q; upnl=mv-cb; tv+=mv; tc+=cb
...
def add_portfolio():
    try:
        d=request.json; uid=d.get('user','default'); sym=d.get('symbol','').upper()
        qty=float(d.get('quantity',0)); ac=float(d.get('avgCost',0))
...
def portfolio_risk():
    return jsonify(safe_dict({'success':True,'risk':{'message':'Risk analizi background modda hesaplaniyor'}}))
...
def add_alert():
    try:
        d=request.json
        a={'id':f"a{len(alert_store)+1}_{int(time.time())}",'email':d.get('email'),'symbol':d.get('symbol','').upper(),'condition':d.get('condition'),'active':True,'createdAt':datetime.now().isoformat()}
...
def get_watchlist():
    uid=request.args.get('user','default')
    stocks=_get_stocks(watchlist_store.get(uid,[]))
...
def backtest():
    return jsonify(safe_dict({'success':True,'message':'Backtest background modda. Lutfen birkaç dakika sonra deneyin.'}))

@app.route('/api/docs')
def docs():
    return jsonify({'name':'BIST Pro v3.0.2','endpoints':['/api/health','/api/dashboard','/api/indices','/api/bist100','/api/bist30','/api/stock/<sym>','/api/screener','/api/portfolio','/api/backtest','/api/alerts','/api/watchlist','/api/sectors','/api/search']})

# =====================================================================
# STARTUP
# =====================================================================
print("[STARTUP] BIST Pro v3.0.2 - RENDER BULLETPROOF")
_loader_thread = threading.Thread(target=_background_loader, daemon=True)
_loader_thread.start()
print("[STARTUP] Background loader baslatildi")
