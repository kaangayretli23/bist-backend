            ...
            try:
                ...
                if not hist.empty and len(hist) >= 2:
                    ...
                    stocks.append({
                        'code': str(code),
                        'name': str(name),
                        'price': float(round(float(current), 2)),
                        'change': float(round(float(change), 2)),
                        'changePct': float(round(float(change_pct), 2))
                    })
            except:
                continue
...
def get_stock_data(symbol):
    ...
    try:
        ...
        # JSON serializable değerlere dönüştür
        result = {
            'success': True,
            'code': str(symbol.upper()),
            'name': str(stock_name),
            'price': float(round(float(current_price), 2)),
            'change': float(round(float(change), 2)),
            'changePercent': float(round(float(change_percent), 2)),
            'volume': int(hist['Volume'].iloc[-1]),
            'dayHigh': float(round(float(hist['High'].iloc[-1]), 2)),
            'dayLow': float(round(float(hist['Low'].iloc[-1]), 2)),
