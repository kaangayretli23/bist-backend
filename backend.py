...
def _fetch_yahoo_http(symbol, period1_days=7):
    """Yahoo Finance v8 chart API - sadece son fiyat icin (background loader)"""
    try:
        now = int(time.time())
...
def _fetch_yahoo_http_df(symbol, period1_days=365):
    """Yahoo Finance v8 chart API - TAM DataFrame dondurur (indicators icin)"""
    try:
        now = int(time.time())
        p1 = now - (period1_days * 86400)
        url = f"https://query1.finance.yahoo.com/v8/finance/chart/{symbol}?period1={p1}&period2={now}&interval=1d"
        ...
        df = pd.DataFrame({
            'Open': [float(v) if v else np.nan for v in opens],
            'High': [float(v) if v else np.nan for v in highs],
            'Low': [float(v) if v else np.nan for v in lows],
            'Close': [float(v) if v else np.nan for v in closes],
        ...
        print(f"  [HTTP-DF] {symbol}: {len(df)} bar, {df.index[0].strftime('%Y-%m-%d')} -> {df.index[-1].strftime('%Y-%m-%d')}")
        return df

    except Exception as e:
        print(f"  [HTTP-DF] {symbol}: {e}")
        return None
...
def stock_detail(symbol):
    ...
    try:
        ...
        if hist is None:
            ...
            # Yontem B: Yahoo HTTP API -> TAM DataFrame
            if hist is None:
                try:
                    print(f"[DETAIL] {symbol} HTTP DataFrame deneniyor...")
                    period_days = {'1mo':30,'3mo':90,'6mo':180,'1y':365,'2y':730,'5y':1825}.get(period, 365)
                    hist = _fetch_yahoo_http_df(f"{symbol}.IS", period_days)
                    if hist is not None and not hist.empty and len(hist) >= 2:
                        _cset(_hist_cache, f"{symbol}_{period}", hist)
                        print(f"[DETAIL] {symbol} HTTP-DF OK: {len(hist)} bar")
                    else:
                        # .IS'siz dene
                        hist = _fetch_yahoo_http_df(symbol, period_days)
                        if hist is not None and not hist.empty and len(hist) >= 2:
                            _cset(_hist_cache, f"{symbol}_{period}", hist)
                            print(f"[DETAIL] {symbol} HTTP-DF2 OK: {len(hist)} bar")
                        else:
                            hist = None
                except Exception as e:
