...
_status = {'phase':'idle','loaded':0,'total':0,'lastRun':None}
CACHE_TTL = 600
...
def _load_one(sym):
    try:
        ...
        return {'code':sym,'name':BIST100_STOCKS.get(sym,sym),'price':cur,'prevClose':prev,
                'change':ch,'changePct':sf(ch/prev*100),'volume':si(h['Volume'].iloc[-1]),
...
def _background_loop():
    ...
    while True:
        try:
            ...
            print(f"[LOADER] Endeksler yukleniyor...")
            for key,(tsym,name) in INDEX_TICKERS.items():
                d = _load_index(key, tsym, name)
                if d:
                    _cset(_index_cache, key, d)
                    print(f"  IDX {key} OK: {d['value']}")
                time.sleep(0.5)

            _status['phase']='stocks'
            _status['total']=len(BIST30)
            _status['loaded']=0
            print(f"[LOADER] {len(BIST30)} hisse yukleniyor...")
            for i,sym in enumerate(BIST30):
                d = _load_one(sym)
                if d:
                    _cset(_stock_cache, sym, d)
                _status['loaded']=i+1
                if (i+1)%10==0:
                    print(f"  STOCKS {i+1}/{len(BIST30)}")
                time.sleep(0.5)

            _status['phase']='done'
            _status['lastRun']=datetime.now().isoformat()
            print(f"[LOADER] OK: {len(_stock_cache)} hisse, {len(_index_cache)} endeks")
        except Exception as e:
            print(f"[LOADER] HATA: {e}")
            _status['phase']='error'
...
def health():
    return jsonify({'status':'ok','version':'3.0.3','yf':YF_OK,'time':datetime.now().isoformat(),'loader':_status,'stockCache':len(_stock_cache),'indexCache':len(_index_cache),'loaderStarted':_loader_started})
...
@app.route('/api/docs')
def docs():
    return jsonify({'name':'BIST Pro v3.0.3','endpoints':['/api/health','/api/dashboard','/api/indices','/api/bist100','/api/bist30','/api/stock/<sym>','/api/screener','/api/portfolio','/api/backtest','/api/alerts','/api/watchlist','/api/sectors','/api/search']})

# NO MODULE-LEVEL THREAD START - before_request handles it
print("[STARTUP] BIST Pro v3.0.3 ready")
