...
def init_db():
    ...
    _migrations = [
        ...
        ("CREATE TABLE IF NOT EXISTS sl_cooldown ("
         ...
         ")", True),
        # Realtime pozisyon uyari state'i (sl_hit/tp1_hit vb. flagler) — restart sonrası
        # tekrar TP1 mesaji atmamak icin persist edilir.
        ("CREATE TABLE IF NOT EXISTS rt_alert_state ("
         "  user_id TEXT NOT NULL,"
         "  symbol TEXT NOT NULL,"
         "  pos_id INTEGER NOT NULL,"
         "  state_json TEXT NOT NULL,"
         "  updated_at REAL NOT NULL,"
         "  PRIMARY KEY (user_id, symbol, pos_id)"
         ")", True),
        # Telegram onay bekleyen sinyaller — restart sonrasinda da gecerli kalmali
        ("CREATE TABLE IF NOT EXISTS pending_signals ("
         "  signal_id TEXT PRIMARY KEY,"
         "  payload_json TEXT NOT NULL,"
         "  expires_at REAL NOT NULL"
         ")", True),
        # Performans index'leri — sik calisan sorgular icin
        # status endpoint: WHERE user_id=? AND status='closed' ORDER BY closed_at DESC
        ("CREATE INDEX IF NOT EXISTS idx_auto_positions_user_status "
         "ON auto_positions(user_id, status, closed_at DESC)", True),
        # trades endpoint: WHERE user_id=? ORDER BY created_at DESC
        ("CREATE INDEX IF NOT EXISTS idx_auto_trades_user_created "
         "ON auto_trades(user_id, created_at DESC)", True),
        # signal_log faktör breakdown kolonu — eski DB'lere ekle (idempotent; varsa hata yutulur)
        ...
        ("CREATE INDEX IF NOT EXISTS idx_signal_log_outcome "
         "ON signal_log(outcome_checked, logged_at)", True),
...
# =====================================================================
# RESTART RESILIENCE — alert_state + pending_signals persistence
# (database_state.py'ye tasindi — modul satir siniri; geriye-uyum icin re-export)
# =====================================================================
from database_state import (
    _db_save_alert_state, _db_delete_alert_state, _db_load_alert_states,
    _db_save_pending_signal, _db_delete_pending_signal, _db_load_pending_signals,
)
